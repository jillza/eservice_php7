...
#----------------------------------------------------------------------------------
# Log Rotator -- LingTalfi -- 2015-10-17 -- https://github.com/lingtalfi/logrotator
#----------------------------------------------------------------------------------
#
# Command Line
# -------------------
    ...
    if [ -f "$logFile" ]; then
        ...
        if [ "$logSize" -ge "$maxSize" ]; then
            ...
            if [ 'd' = "$lastChar" ]; then
                ...
                # note: I used minutes for quick testing
                nbMin=$(( rotateValue * 24 * 60 ))
		find "$logDir" -type f -mmin +${nbMin} -delete
            else
